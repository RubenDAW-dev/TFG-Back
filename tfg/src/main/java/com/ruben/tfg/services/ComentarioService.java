...
public class ComentarioService {
	...
	private void validarTitulo(CrearComentarioDTO req) {
		...
		if (req.getComentarioPadreId() == null) {
			if (req.getTitulo() == null || req.getTitulo().isBlank()) {
				throw new IllegalArgumentException("Los topics del foro deben tener un título.");
			}
		}
	}
	...
	public List<ComentarioResponseDTO> listarPorPartido(Long matchId) {
		return comentarioRepository.findByPartidoIdAndComentarioPadreIsNull(matchId).stream().map(this::toDTO)
				.collect(Collectors.toList());
	}

	public List<ComentarioResponseDTO> listarPorEquipo(String teamId) {
		return comentarioRepository.findByEquipoIdAndComentarioPadreIsNull(teamId).stream().map(this::toDTO)
				.collect(Collectors.toList());
	}

	public List<ComentarioResponseDTO> listarPorJugador(String playerId) {
		return comentarioRepository.findByJugadorIdAndComentarioPadreIsNull(playerId).stream().map(this::toDTO)
				.collect(Collectors.toList());
	}
	...
	public List<ComentarioResponseDTO> listarTopicsForo() {
		return comentarioRepository.findByComentarioPadreIsNullOrderByFechaDesc().stream().map(this::toDTOResumen)
				.collect(Collectors.toList());
	}
	...
	public List<ComentarioResponseDTO> respuestasDe(Integer comentarioId) {
		ComentarioEntity padre = comentarioRepository.findById(comentarioId)
				.orElseThrow(() -> new EntityNotFoundException("Comentario no encontrado: " + comentarioId));
		return padre.getRespuestas().stream().map(this::toDTO).collect(Collectors.toList());
	}
	...
	public ComentarioResponseDTO obtenerTopic(Integer id) {
		ComentarioEntity c = comentarioRepository.findById(id)
				.orElseThrow(() -> new EntityNotFoundException("Comentario no encontrado: " + id));
		return toDTO(c);
	}
	...
	private ComentarioResponseDTO toDTO(ComentarioEntity c) {
		...
		dto.setRespuestas(c.getRespuestas().stream().map(this::toDTO).collect(Collectors.toList()));
	...
	public ComentarioResponseDTO actualizarComentario(actualizarComentarioDTO req) {
		...
		ComentarioEntity comentario = comentarioRepository.findById(req.getId())
				.orElseThrow(() -> new EntityNotFoundException("Comentario no encontrado: " + req.getId()));
